...
final class MarkerVisibilityController: NSObject, NSLayoutManagerDelegate {
    ...
    private var bulletRanges: [NSRange] = []

    /// Index des caractères effectivement masqués, recalculés à chaque
    /// changement de texte ou de sélection.
    private var hidden = IndexSet()
    private var bullets = IndexSet()

    private(set) var selection = NSRange(location: 0, length: 0)
    private var bulletGlyphCache: [String: CGGlyph] = [:]

    // MARK: - Mise à jour

    func update(markers: [MarkdownHighlighter.MarkerRange], bullets bulletRanges: [NSRange], enabled: Bool) {
        self.markers = markers
        self.bulletRanges = bulletRanges
    ...
    private func recompute() {
        ...
        if isEnabled {
            ...
            for range in bulletRanges where range.length == 1 {
                newBullets.insert(range.location)
            }
        }
    ...
    func layoutManager(
        ...
        guard isEnabled, !hidden.isEmpty || !bullets.isEmpty else { return 0 }
        ...
        for i in 0 ..< count {
            ...
            if hidden.contains(characterIndex) {
                newProps[i] = .null
                changed = true
            } else if bullets.contains(characterIndex), let glyph = bulletGlyph(for: font) {
                newGlyphs[i] = glyph
                changed = true
            }
        }
    ...
    /// Glyphe « • » dans la fonte courante, mis en cache par fonte.
